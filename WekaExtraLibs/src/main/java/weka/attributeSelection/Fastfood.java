...
//        System.out.print(" finished.\n");
    ...
    protected void determineNumToSelectFromThreshold(double[][] ranking) {
        int count = 0;
        for (double[] aRanking : ranking) {
            if (aRanking[1] > m_threshold) {
                count++;
            }
        }
    ...
    public String toString() {
        ...
        if (m_threshold != -Double.MAX_VALUE) {
            ...
            BfString.append(Utils.doubleToString(m_threshold, 8, 4)).append("\n");
        }
    ...
    protected boolean inStarting(int feat) {
        ...
        if (m_hasClass && feat == m_classIndex) {
            return false;
        }
        ...
        for (int aM_starting : m_starting) {
            if (aM_starting == feat) {
                return true;
            }
        }
